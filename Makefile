...
build/release-pi/%: $(GOFILES)
	@mkdir -p "$(@D)"
	@echo "   \x1b[1;34mgo build \x1b[0;1m(release-pi)\x1b[0m  $@"
	@GOOS=linux GOARCH=arm GOARM=6 $(GO) build -ldflags="-s -w" -trimpath -i -o "$@" -gcflags "$(GCFLAGS_RELEASE_PI)" "$(MODULE)/cmd/$(@F)"

build/release-pi1/%: $(GOFILES)
	@mkdir -p "$(@D)"
	@echo "   \x1b[1;34mgo build \x1b[0;1m(release-pi1)\x1b[0m  $@"
	@GOOS=linux GOARCH=arm GOARM=5 $(GO) build -ldflags="-s -w" -trimpath -i -o "$@" -gcflags "$(GCFLAGS_RELEASE_PI1)" "$(MODULE)/cmd/$(@F)"
